...
**Severity:** SEV2
**Owner (incident raised on):** Payments Engineering → **handed off to Core Platform**
...
binding constraint was the shared DB connection pool — owned by Core Platform.
We escalated; Core Platform found a pool-sizing change and fixed it.
...
- 14:37 — Dominant log error = pgbouncer pool exhaustion. **Escalated to Core
  Platform** with the incident, the pool-exhaustion log lines, and the deploy
  timeline.
- 14:44 — Core Platform identified an undersized pgbouncer pool and reverted it.
...
- For checkout-api 5xx, **decide ownership before deep-diving.** If 5xx predates
  ...
  the correct action is to **escalate to Core Platform** — not to keep digging in
...
- Added an alert on pgbouncer pool utilization (requested from Core Platform).
