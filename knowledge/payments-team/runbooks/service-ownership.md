...
## We consume but DO NOT own → escalate to Infra Team

| Component | Owner | Common failure we mis-attribute to ourselves |
|---|---|---|
| billing-postgres | Infra Team | `could not obtain connection from pool` |
| **pgbouncer** (pooler) | Infra Team | `pool 'billing' is full (N/N)` — pool-size too low |
| kafka-billing-events | Infra Team | consumer lag (often downstream of the DB pool) |
...
## When to hand off to Infra Team
...
change. Infra Team owns the fix (pool sizing); we cannot and should not change
pgbouncer ourselves.
